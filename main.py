...
class GroundAllRules:
    ...
    def PropositionalizeRule(self, fnEntityIDMap, fnRelationIDMap, fnRuleType, fnTrainingTriples, fnOutout):
        self.readData(fnEntityIDMap, fnRelationIDMap, fnTrainingTriples)
        self.groundRule(fnRuleType, fnOutout)
            ...
            for line in f.readlines():
                ...
                if len(bodys) == 3:
                    ...
                    for i in range(0, iSize):
                        ...
                        strKey = "(" + str(iSubjectID) + "\t" + str(iFstRelation) + "\t" + str(iObjectID) +")\t" + "(" + str(self.MapVariable[bEntity3]) + "\t" + str(iSndRelation) + "\t" + str(self.MapVariable[bEntity4]) + ")"
                        strCons = str(self.MapVariable[bEntity3]) + "\t" + str(iSndRelation) + "\t" + str(self.MapVariable[bEntity4])
                        ...
                        for iFstIndex in range(0, iFstSize):
                            ...
                            for iSndIndex in range(0, iSndSize):
                                ...
                                for iTrdIndex in range(0, iTrdSize):
                                    ...
                                    strKey = "(" + str(lstEntity1) + "\t" + str(iFstRelation) + "\t" + str(iEntity2ID) + ")\t(" + str(iEntity3ID) + "\t" + str(iSndRelation) + "\t" + str(iEntity4ID) + ")\t" + "(" + str(self.MapVariable[hEntity1]) + "\t" + str(iTrdRelation) + "\t" + str(self.MapVariable[hEntity2]) + ")"
...
if __name__ == "__main__":
    ...
    groundAllRules = GroundAllRules()
    groundAllRules.readData("entities.dict", "relations.dict", "train.txt")
    groundAllRules.groundRule("fb15k_rule", "grounding.txt")
